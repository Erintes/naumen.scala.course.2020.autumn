object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        ...
        for(i <- 2 to Math.sqrt(divNum).toInt + 1)
            while (divNum % i == 0){
                if (!res.contains(i)) res :+= i
                divNum /= i
            }

        if (divNum != 1) res :+= divNum
        res
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        val sortedMap = ballsArray.toSeq.sortBy(x => x._2._2 * x._2._1 * x._2._1 * Math.PI).toMap
        sortedMap.keys.toSeq
    }
